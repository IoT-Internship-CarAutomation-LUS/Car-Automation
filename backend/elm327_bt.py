...
#   python elm327_bt.py --tcp <ip>      : Also forward raw 32-byte packets to laptop_ip:9000
#                                          (bench-test link, separate from --stream)
#
...
TCP_PORT        = 9000                       # Bench-test raw packet port (--tcp <laptop_ip>)
...
def run_capture(raw_mode: bool = False, fast_mode: bool = False, stream_mode: bool = False, tcp_target: str = None):
    ...
    # --tcp: separate, simpler direct link for bench testing -- raw 32-byte
    # packets straight to a laptop script (raw_receiver.py / decoded_receiver.py),
    # independent of --stream and the backend WebSocket path.
    tcp_sock = None
    if tcp_target:
        try:
            tcp_sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            tcp_sock.connect((tcp_target, TCP_PORT))
            print(f"[TCP] --tcp enabled. Connected to {tcp_target}:{TCP_PORT}")
        except OSError as e:
            print(f"[TCP] Connect failed ({e}) -- continuing offline capture.")
    ...
    try:
        while True:
            ...
            if tcp_target:
                print(f"[OBD] --tcp modifier enabled: forwarding raw packets to {tcp_target}:{TCP_PORT}")
            ...
            try:
                while True:
                    ...
                    # --tcp: best-effort raw packet forward, same failure handling
                    # philosophy as --stream -- log a warning and keep capturing locally.
                    if tcp_target and tcp_sock is not None:
                        try:
...
if __name__ == "__main__":
    ...
    if "--tcp" in sys.argv:
        ...
        if idx + 1 >= len(sys.argv):
            print("[TCP] ERROR: --tcp requires a laptop IP argument, e.g. --tcp 192.168.1.20")
            sys.exit(1)
        tcp_target = sys.argv[idx + 1]
    ...
    if scan_mode:
        ...
        print(f"  TCP link : {tcp_target + ':' + str(TCP_PORT) if tcp_target else 'DISABLED'}")
        print(f"  Modes    : stream={stream_mode}, raw={raw_mode}, fast={fast_mode}")
        print("=" * 55)
        try:
            run_capture(raw_mode=raw_mode, fast_mode=fast_mode, stream_mode=stream_mode, tcp_target=tcp_target)
